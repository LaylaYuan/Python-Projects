...
c = conn.cursor()
c.execute('''CREATE TABLE IF NOT EXISTS GOOG( stock TEXT, date INT, compound_score FLOAT, positive_counts INT, negative_counts INT, neural_counts INT, p_sentiment FLOAT, n_sentiment FLOAT, total_sentiment FLOAT) ''')
c.execute('delete from GOOG')
conn.commit()
...
for date in time_list:
    url = 'https://newsapi.org/v2/everything?q=google&from=%s&to=%s&language=en&apiKey=d74cd7c10e6145be8527a8b18cbaff60'%(date,date)
...
for i in range(len(stock_sql)):
    ...
    c.execute('''INSERT INTO GOOG(stock, date, compound_score, positive_counts, negative_counts, neural_counts, p_sentiment, n_sentiment, total_sentiment) values(?,?,?,?,?,?,?,?,?)''', collect_data)
    #print (collect_data)
